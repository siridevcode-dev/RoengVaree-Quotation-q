...
export default function HRManagement({ activeTab: initialTab = "พนักงาน" }: HRManagementProps) {
  ...
  const subTabs = [
    { id: "พนักงาน", label: "พนักงาน", count: null },
    { id: "ตารางการทำงาน", label: "ตารางการทำงาน", count: null },
    { id: "หนังสือเตือน", label: "หนังสือเตือน", count: null },
    { id: "ขอเข้าร่วมบริษัท", label: "ขอเข้าร่วมบริษัท", count: 0 },
  ];
  ...
  const renderContent = () => {
    if (showAddForm) {
      return <HREmployeeForm onBack={() => setShowAddForm(false)} />;
    }

    switch (activeSubTab) {
      case "พนักงาน":
        return <HREmployeeList onAddIndividual={() => setShowAddForm(true)} />;
      case "ตารางการทำงาน":
          ...
          <div className="flex items-center gap-2">
            {subTabs.map((tab) => (
              <button title="ปุ่ม"
                ...
                className={`relative px-4 py-4 text-[14px] font-bold transition-all group ${
                  activeSubTab === tab.id ? "text-[#0095FF]" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                <div className="flex items-center gap-1.5">
                  <span>{tab.label}</span>
                  {tab.count !== null && (
                    <span className={`px-2 py-0.5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                      activeSubTab === tab.id ? "bg-[#E6F4FF] text-[#0095FF]" : "bg-slate-100 text-slate-400"
                    }`}>
                      {tab.count}
                    </span>
                  )}
                ...
                {activeSubTab === tab.id && (
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#0095FF]" />
                )}
              </button>
            ))}
          </div>
